...
function renderMarkdown(content) {
  return marked(normalizeReportMarkdown(content));
}
...
function renderReportPage(report, res) {
  ...
  const title = extractReportTitle(report.content) || `每日新闻日报 - ${report.date}`;
...
app.get('/archives', (req, res) => {
  ...
  const reportsHtml = archives.map((entry) => {
    ...
    const preview = extractReportTitle(entry.content) || '点击查看详细内容';
    return `
